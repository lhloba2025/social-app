...
export default function SocialPanel({ box, onUpdate, language }) {
  ...
  return (
    <div className="space-y-3 p-3" dir={isRtl ? "rtl" : "ltr"}>
      {/* Header + show/hide */}
      <div className="bg-[var(--hv-surface-2)] border border-cyan-300 rounded-lg p-3 space-y-2">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-bold flex items-center gap-2" style={{ color: "var(--hv-text)" }}>
          ...
          <label className="flex items-center gap-1.5 cursor-pointer">
            ...
            <span className="text-[10px] text-slate-600">{isRtl ? "إظهار" : "Show"}</span>
          </label>
        </div>
      ...
      <div className={`rounded-lg p-2.5 border text-[11px] transition ${
        restoredFlash
          ...
          : profileExists
            ...
            : "bg-[var(--hv-surface-2)] border-slate-200 text-slate-500"
      }`}>
        ...
        {profileExists && (
          <div className="flex flex-wrap gap-1 mt-1.5 pt-1.5 border-t border-emerald-200">
            <button
              ...
              className="flex items-center gap-1 text-[10px] px-2 py-0.5 rounded bg-emerald-100 hover:bg-emerald-200 text-emerald-700"
            >
            ...
            <button
              ...
              className="flex items-center gap-1 text-[10px] px-2 py-0.5 rounded bg-slate-100 hover:bg-red-100 text-slate-600 hover:text-red-600"
            >
              <X className="w-3 h-3" />
      ...
      {/* Platform picker */}
      <div>
        <label className="text-[10px] text-slate-500 block mb-1.5">{isRtl ? "أضف منصة:" : "Add a platform:"}</label>
        <div className="grid grid-cols-6 gap-1.5">
          {SOCIAL_PLATFORMS.map((p) => (
            <button key={p.id}
              onClick={() => addItem(p.id)}
              title={isRtl ? p.nameAr : p.nameEn}
              className="aspect-square rounded-lg overflow-hidden border border-slate-200 bg-white hover:scale-105 hover:border-cyan-400 transition"
            >
        ...
        <div>
          ...
          <div className="space-y-2">
            {box.items.map((it, idx) => {
              ...
              if (!p) return null;
              return (
                <div key={it.id} className="bg-[var(--hv-surface-2)] border border-slate-200 rounded p-2 space-y-1.5">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <input
                      ...
                      dir="ltr"
                      className="hv-input flex-1 px-2 py-1 text-[11px] min-w-0"
                    />
                  </div>
                  ...
                  <div className="flex items-center gap-1 text-[10px]">
                    ...
                    <button onClick={() => moveItem(it.id, -1)}
                      ...
                      className="text-slate-400 hover:text-indigo-600 disabled:opacity-30 px-1.5 rounded">▲</button>
                    <button onClick={() => moveItem(it.id, +1)}
      ...
      {box.items.length > 0 && (
        <div className="bg-[var(--hv-surface-2)] border border-slate-200 rounded-xl p-3 space-y-3">
          <h3 className="text-sm font-bold flex items-center gap-2" style={{ color: "var(--hv-text)" }}>
          ...
          {/* Layout */}
          <div>
            <label className="text-[10px] text-slate-500 block mb-1">{isRtl ? "ترتيب" : "Layout"}</label>
              ...
              {[
                ...
                <button key={m.id}
                  ...
                  className={`py-1.5 rounded text-[10px] font-semibold transition ${
                    box.layout === m.id ? "bg-cyan-500 text-white" : "bg-white border border-slate-200 text-slate-600 hover:bg-slate-100"
                  }`}>
                  {isRtl ? m.ar : m.en}
                </button>
              ))}
          ...
          {/* Colour mode */}
          <div>
            <label className="text-[10px] text-slate-500 block mb-1">{isRtl ? "نمط الألوان" : "Color mode"}</label>
            <div className="grid grid-cols-3 gap-1">
              {[
                ...
                <button key={m.id}
                  ...
                  className={`py-1.5 rounded text-[10px] font-semibold transition ${
                    box.colorMode === m.id ? "bg-cyan-500 text-white" : "bg-white border border-slate-200 text-slate-600 hover:bg-slate-100"
                  }`}>
                  {isRtl ? m.ar : m.en}
                </button>
              ))}
          ...
          {box.colorMode === "mono" && (
            <div className="flex items-center gap-2">
              <input type="color" value={box.monoColor}
                ...
                className="w-8 h-8 rounded cursor-pointer bg-white border border-slate-200" />
              <label className="text-[10px] text-slate-500 flex-1">{isRtl ? "لون الأيقونات" : "Icon color"}</label>
            </div>
          )}

          {/* Sizing */}
          <div>
            <label className="text-[10px] text-slate-500 block">{isRtl ? "حجم الأيقونة" : "Icon size"}: {box.iconSize}%</label>
            <input type="range" min="3" max="20" step="0.5" value={box.iconSize}
              onChange={(e) => onUpdate({ iconSize: parseFloat(e.target.value) })}
              className="w-full accent-cyan-500" />
          </div>
          <div>
            <label className="text-[10px] text-slate-500 block">{isRtl ? "المسافة" : "Spacing"}: {box.spacing}%</label>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-[10px] text-slate-600">{isRtl ? "إظهار المعرفات بجانب الأيقونات" : "Show handles next to icons"}</span>
          </label>
          {box.showLabels && (
            <div className="flex items-center gap-2">
              <input type="color" value={box.textColor}
                ...
                className="w-8 h-8 rounded cursor-pointer bg-white border border-slate-200" />
              <label className="text-[10px] text-slate-500 flex-1">{isRtl ? "لون النص" : "Text color"}</label>
            </div>
          )}
          ...
          <div className="bg-white border border-slate-200 rounded-lg p-2 space-y-2">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-[11px] font-bold" style={{ color: "var(--hv-text)" }}>
            ...
            {box.bgEnabled && (
              <>
                ...
                <div className="grid grid-cols-2 gap-1 bg-slate-100 rounded p-1">
                  <button onClick={() => onUpdate({ bgMode: "solid" })}
                    className={`py-1 rounded text-[10px] font-bold transition ${
                      (box.bgMode || "solid") === "solid" ? "bg-cyan-500 text-white" : "text-slate-600 hover:bg-slate-200"
                    }`}>
                    {isRtl ? "لون واحد" : "Solid"}
                  </button>
                  <button onClick={() => onUpdate({ bgMode: "gradient" })}
                    className={`py-1 rounded text-[10px] font-bold transition ${
                      box.bgMode === "gradient" ? "bg-cyan-500 text-white" : "text-slate-600 hover:bg-slate-200"
                    }`}>
                  ...
                  <>
                    <div className="flex items-center gap-2">
                      <input type="color" value={box.bgGradColor2 || "#1e293b"}
                        ...
                        className="w-8 h-8 rounded cursor-pointer bg-white border border-slate-200" />
                      <label className="text-[10px] text-slate-500 flex-1">{isRtl ? "اللون 2" : "Color 2"}</label>
                    </div>
                    <div>
                      <label className="text-[10px] text-slate-500 block">{isRtl ? "اتجاه التدرّج" : "Angle"}: {box.bgGradAngle || 135}°</label>
                    ...
                    <div>
                      <label className="text-[10px] text-slate-500 block mb-1">{isRtl ? "تدرّجات جاهزة" : "Presets"}</label>
                      <div className="grid grid-cols-3 gap-1">
                        {[
                          ...
                          <button key={i}
                            ...
                            className="h-6 rounded border border-slate-200 hover:scale-105 transition"
                ...
                {box.bgMode !== "gradient" && (
                  <div>
                    <label className="text-[10px] text-slate-500 block">{isRtl ? "الشفافية" : "Opacity"}: {Math.round((box.bgOpacity ?? 1) * 100)}%</label>
                ...
                <div>
                  <label className="text-[10px] text-slate-500 block">{isRtl ? "استدارة الزوايا" : "Corner radius"}: {box.bgRadius}%</label>
                  <input type="range" min="0" max="80" step="2" value={box.bgRadius}
                    onChange={(e) => onUpdate({ bgRadius: parseFloat(e.target.value) })}
                    className="w-full accent-cyan-500" />
                </div>
                <div>
                  <label className="text-[10px] text-slate-500 block">{isRtl ? "الحشو الداخلي" : "Padding"}: {box.bgPadding}%</label>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-[10px] text-slate-500 block">X: {box.x.toFixed(1)}%</label>
              <input type="range" min="0" max="100" step="0.5" value={box.x}
                onChange={(e) => onUpdate({ x: parseFloat(e.target.value) })}
                className="w-full accent-cyan-500" />
            </div>
            <div>
              <label className="text-[10px] text-slate-500 block">Y: {box.y.toFixed(1)}%</label>
          ...
          <div>
            <label className="text-[10px] text-slate-500 block">{isRtl ? "دوران" : "Rotation"}: {box.rotation || 0}°</label>
