...
export default function HoveraLogo({ size = 40, withText = true, className = '', ar = true }) {
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      {imgOk ? (
        ...
        >
          <span className="font-extrabold text-white" style={{ fontSize: size * 0.4 }}>{ar ? 'هـ' : 'H'}</span>
        </div>
      )}
      {withText && (
